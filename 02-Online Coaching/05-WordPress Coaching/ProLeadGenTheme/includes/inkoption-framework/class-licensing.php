...
class InkThemes_License extends InkOption_Framwork {
	...
	function admin_license_notice() {
		...
		if ( empty( $this->key ) && str_replace( 'inkthemes_page_', '', $currentScreen->id ) != 'inkoption-setting' ) {
			$message = sprintf( '<p><strong>%s</strong></p>', __( "Your theme has not been activated yet.", 'black-rider' ) );
			$message .= sprintf( '<p>%s <a href="' . admin_url( '/admin.php?page=inkoption-setting' ) . '">%s</a></p>', __( "Please pick the license from members area and activate it", 'black-rider' ), __( 'from here', 'black-rider' ) );
	...
	function license_check( $key ) {
		if ( empty( $key ) ) {
			$this->errorMsg = __( 'Please enter a license key', 'black-rider' );
			return false;
		} else {
	...
	function license_reset_callback() {
		...
		if ( $is_delete ) {
			...
			_e( 'License has been reseted', 'black-rider' );
		}
		die();
	}
	...
	function license_verify_callback() {
		...
		if ( !wp_verify_nonce( $auth_data['inkthemes_license_nonce'], 'inkthemes_license' ) ) {
			$this->is_error = true;
			$response['error'] = __( 'There is some error.', 'black-rider' );
		}

		$key = $auth_data['inkthemes_license_key'];

		if ( empty( $key ) ) {
			$response['error'] = __( 'Please enter a license key', 'black-rider' );
			$this->is_error = true;
		}
		...
		if ( !$this->is_error ) {
			$response['activated'] = __( 'Your license has been activated.', 'black-rider' );
		}
