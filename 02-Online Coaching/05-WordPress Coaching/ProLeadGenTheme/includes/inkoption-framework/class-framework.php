...
class InkOption_Framwork {
    ...
    const THEME_SLUG = 'black-rider';
    const THEME_NAME = 'BlackRider pro';
    ...
    function scripts() {
        ...
        if (str_replace('inkthemes_page_', '', $currentScreen->id) == 'inkoption-framework') {
        ...
        if (str_replace('inkthemes_page_', '', $currentScreen->id) == 'inkoption-setting') {
            wp_enqueue_style('inkoption-settingstyle', $this->path_uri . 'css/setting-style.css');
            wp_enqueue_script('inkoption-license-js', $this->path_uri . 'js/license-activation.js', array('jquery'));
        }
    }
    ...
    function js() {
        ...
        if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'inkoption-framework') {
            $ajax_type = 'options';
        } else {
            $ajax_type = '';
        }
    ...
    function framework_page() {
        add_menu_page(__('InkThemes', 'black-rider'), __('InkThemes', 'black-rider'), 'edit_theme_options', 'inkthemes-framework', array($this, 'framework_interface'), $this->path_uri . 'images/setting-icon.png', 58);
        //add_submenu_page( 'inkthemes-framework', __( 'Theme Activation', 'black-rider' ), __( 'Theme Activation', 'black-rider' ), 'edit_theme_options', 'inkoption-setting', array( $this, 'framework_interface' ) );
        add_submenu_page('inkthemes-framework', __('Theme Options', 'black-rider'), __('Theme Options', 'black-rider'), 'edit_theme_options', 'inkoption-framework', array($this, 'theme_option_interface'));
        remove_submenu_page('inkthemes-framework', 'inkthemes-framework');
    }
    ...
    function reset() {
        if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'inkoption-framework') {
            if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
                ...
                $this->reset_options($options, 'inkoption-framework');
                header("Location: admin.php?page=inkoption-framework&reset=true");
                die;
            }
        }
    }
        ...
        if ($save_type == 'upload') {
            ...
            if (isset($uploaded_file) && ($uploaded_file['type'] == 'image/gif' || $uploaded_file['type'] == 'image/jpeg' || $uploaded_file['type'] == 'image/pjpeg' || $uploaded_file['type'] == 'image/png' || $uploaded_file['type'] == 'image/svg+xml' ) || $uploaded_file['type'] == 'image/x-icon') {
                ...
                if (!empty($uploaded_file['error'])) {
                    $response['error'] = __('Upload Error: ', 'black-rider') . $uploaded_file['error'];
                } else {
                    $response['url'] = $uploaded_file['url'];
                }
            } else {
                $response['error'] = __('Unsupported filetype uploaded.', 'black-rider');
            } // Is the Response
...
//				wp_die( __( 'Your theme license is not activated. Please activate it from <a href="' . admin_url( '/admin.php?page=inkoption-setting' ) . '">here</a> to get complete option of this theme.', 'black-rider' ) );
//			}
//		}
        $counter = 0;
        $menu = '';
    ...
    function options_admin_head() {
        ...
        <script type="text/javascript">
            jQuery(function () {
                ...
                var message = '<p><?php _e('This theme comes with an ', 'black-rider'); ?><a href="<?php echo admin_url('admin.php?page=inkoption-framework'); ?>"><?php _e('options panel</a> to configure settings. This theme also supports widgets, please visit the ', 'black-rider'); ?><a href="<?php echo admin_url('widgets.php'); ?>"><?php _e('widgets settings page</a> to configure them.', 'black-rider'); ?></p>';
