...
// create email headers
    ...
    public function table_update($ulabel, $rname) {
        ...
        if ($ulabel == 'Name') {
            $wpdb->update($table_info, array('lead_name' => $rname), array('id' => 1), array('%s'), array('%d'));
        }
        if ($ulabel == 'Email') {
            $wpdb->update($table_info, array('lead_name' => $rname), array('id' => 2), array('%s'), array('%d'));
        }
        if ($ulabel == 'Number') {
            $wpdb->update($table_info, array('lead_name' => $rname), array('id' => 3), array('%s'), array('%d'));
        }
        if ($ulabel == 'Message') {
            $wpdb->update($table_info, array('lead_name' => $rname), array('id' => 4), array('%s'), array('%d'));
        }
        if ($ulabel == 'label1') {
            $wpdb->update($table_info, array('lead_name' => $rname), array('id' => 5), array('%s'), array('%d'));
        }
        if ($ulabel == 'label2') {
            $wpdb->update($table_info, array('lead_name' => $rname), array('id' => 6), array('%s'), array('%d'));
        }
        if ($ulabel == 'label3') {
            $wpdb->update($table_info, array('lead_name' => $rname), array('id' => 7), array('%s'), array('%d'));
        }
        if ($ulabel == 'label4') {
            $wpdb->update($table_info, array('lead_name' => $rname), array('id' => 8), array('%s'), array('%d'));
        }
        if ($ulabel == 'label5') {
            $wpdb->update($table_info, array('lead_name' => $rname), array('id' => 9), array('%s'), array('%d'));
        }
        if ($ulabel == 'label6') {
            $wpdb->update($table_info, array('lead_name' => $rname), array('id' => 10), array('%s'), array('%d'));
        }
    }
