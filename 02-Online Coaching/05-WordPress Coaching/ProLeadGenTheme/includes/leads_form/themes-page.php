...
function enqueue_styles() {
    ...
    wp_register_style($themeslug . 'storecss', get_template_directory_uri() . '/includes/theme-page-style.css');
    wp_enqueue_style($themeslug . 'storecss');
}

// Add page to the menu
function blackrider_add_menu() {
    $page = add_theme_page('InkThemes Themes Page', 'Leads Capture', 'administrator', 'themes', 'blackrider_page_init');
    add_action('admin_print_styles-' . $page, 'enqueue_styles');
}

add_action('admin_menu', 'blackrider_add_menu');

// Create the page
function blackrider_page_init() {
    ...
    <div id="contain">
        ...
        $sql = $wpdb->get_results("SELECT * FROM $inkthemes_table", ARRAY_A);
        $nr = $wpdb->get_var("SELECT COUNT(*) FROM $inkthemes_table");
        if ($nr >= 1) {
            if (isset($_GET['pn'])) {
            ...
            $sql2 = $wpdb->get_results("SELECT * FROM $inkthemes_table ORDER BY id DESC $limit", ARRAY_A);
            $paginationDisplay = "";
            if ($lastPage != "1") {
            ...
            <table class="wp-list-table widefat fixed pages">
                <thead>
                    <tr>
                        ...
                        <?php
                        global $table_info, $wpdb;
                        $sqlfeatch = $wpdb->get_results("SELECT * FROM $table_info", ARRAY_A);
...
//data fetch for dashboard 
                    ...
                    foreach ($sql2 as $row) {
                        ...
                        <tbody id="trans_list">
                            <tr class="data">   
                                ...
                                <th  scope="col"><a href="<?php echo get_permalink() . "?page=themes&pn=" . $pn . "&delete=" . $id; ?>"><img src="<?php echo $root; ?>/includes/images/ico-close.png" alt="Delete" height="16" width="16" /></a></th>
                            </tr>
                        <tbody id="trans_list">
                            <?php
                        }
            ...
            <table class="wp-list-table widefat fixed pages" style="width:180px;">
                <thead><tr> <th  scope="col" style="text-align:center;">Download CSV File</th>		 
                        ...
                        <th  scope="col" style="text-align:center;"> <a id="csvfile" href="<?php echo $importleads; ?>" alt="Download CSV File"><img src="<?php echo $root; ?>/includes/images/export.png" alt="Download CSV File" height="32" width="32" /></a>	</th>
            </table> </thead>
    </div>        
    </div>	 </div>
    </div>
