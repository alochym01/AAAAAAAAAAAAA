        ...
        <div class="row">
            <div class="col-md-12">
                <div class="fullwidth">                
                    ...
                    <?php
                    printf(__('By %2$s', 'black-rider'), 'meta-prep meta-prep-author', sprintf('<a class="url fn n" href="%1$s" title="%2$s">%3$s</a>', get_author_posts_url(get_the_author_meta('ID')), sprintf(esc_attr__(__('View all posts by %s', 'black-rider')), get_the_author()), get_the_author()
                    ));
                    ?>
                    <span>|</span>
                    <?php
                    ...
                    if (wp_attachment_is_image()) {
                        ...
                        printf(__('Full size is', 'black-rider'), sprintf('<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>', wp_get_attachment_url(), esc_attr(__('Link to full-size', 'black-rider')), $metadata['width'], $metadata['height']));
                    }

                    edit_post_link(__('Edit', 'black-rider'), '', '');
                    ?>
                    <!-- .entry-meta -->
                    <?php
                    if (wp_attachment_is_image()) {
                    ...
                    the_content(__('Continue reading &rarr;', 'black-rider'));
                    wp_link_pages(array('before' => '' . __('Pages:', 'black-rider'), 'after' => ''));
                    blackrider_posted_in();
                    edit_post_link(__('Edit', 'black-rider'), ' ', '');
                    comments_template();
                    ?>
                </div> 
